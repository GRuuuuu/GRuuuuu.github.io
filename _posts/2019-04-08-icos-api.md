...
![image](https://user-images.githubusercontent.com/15958325/55781656-a8f2b600-5ae5-11e9-9b06-f4eb5a0da5d4.png)  

에러가나는 부분은 `.then`, 아무래도 node의 버전차이인듯 싶습니다.  

IBM cloud는 사용자가 임의로 node의 버전을 변경할 수 없으니 사실상 getGalleryImages(2)의 소스코드를 본 튜토리얼에서 사용하는건 불가능하고, 현재(19/04/09)는 getGalleryImages(1)를 사용해야합니다.  

하지만 추후에 IBM cloud의 업데이트됨에 따라 `node sdk`버전이 올라간다면 getGalleryImages(2)를 사용할수있을테니 그때는 최-신 `aws-sdk`를 사용하시는것을 추천드립니다.  
