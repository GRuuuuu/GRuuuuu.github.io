...
k8s클러스터는 1마스터 2슬레이브로 구성하겠습니다.  

`Master` : 4cpu, ram16G  
`Slave` : 2cpu, ram4G  

# 3. Steps
>(M,S) : Master와 Slave 노드 전부 적용

## 3.1 Set Hostname on Nodes (M,S)

각 Master, Slave노드들마다 자신의 hostname을 결정해줍니다.  
...
## 3.3 Configure Firewall (M,S)
방화벽을 끄지 않고 마스터노드와 슬레이브노드의 포트를 설정할수는 있지만, 이번 문서에서는 그냥 방화벽을 끄고 진행하겠습니다.  
...
## 3.4 Update Iptables Settings (M,S)
iptable 설정을 해줍니다.  
~~~sh
...
## 3.8 kubelet, kubeadm, kubectl 설치 (M,S)
~~~sh
...
위 명령어를 실행해 마스터노드를 초기화시키고나면 성공적으로 초기화 시켰다는 로그와 함께 슬레이브노드가 마스터노드에 join할 수 있게 하는 커맨드가 뜨게 됩니다.  
...
## 3.10 Slave노드 join (S)
위에 Master노드를 초기화시킬때 복사해둔 `kubeadm join~`을 사용할 차례입니다.  

복사한 명령어를 각 slave노드로 가서 입력해줍니다.  
~~~sh
kubeadm join x.x.x.x:6443 --token b0em0n.b8v9b760cmz8nmty \
    --discovery-token-ca-cert-hash sha256:dd5ae0bb6c4f6e7aff5f8ac34d63383d50e1f374a36c9530cec756c406bec4e7
...
성공적으로 join했다는 메세지가 뜨면 마스터 노드로 가서 확인해봅시다.  
...
마스터 노드 하나에 슬레이브 노드 두개로 클러스터가 구성된 것을 확인할 수 있습니다.  
...
>이때 ip는 마스터의 ip이든 슬레이브의 ip든 상관없습니다.  
...
Master노드에서 :  
~~~sh
$ kubectl drain {slave노드이름} --delete-local-data --force --ignore-daemonsets
$ kubectl delete node {slave노드이름}
