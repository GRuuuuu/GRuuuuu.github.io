...
그래서 인터페이스를 단일화 시킴과 동시에 각 노드는 인터넷 연결이 되게 해야합니다.  

제가 해결한 방법은 호스트전용어댑터만 살리고 network통신을 ocp-dns(bastion)으로 라우트시키는 방법입니다.  
...
>VirtualBox에 익숙치 않고 networking에 대한 지식이 얕아서 이런 방법을 사용한것인데 이것보다 깔끔한 방법이 있을것입니다. 각 노드가 네트워크통신이 되고, 노드끼리 통신이 가능하기만 하면 되니 이부분은 참고만 해주세요.  
